(function() {
  ...
  var go = function(p) {
    ...
    if (dd.ios) { //ios compat
      ...
      if (p.createIfNeeded) {
        if (!url && (!p.url || typeof p.url !== 'string')) throw 'url of go() is missing or invalid';

        url = p.url;
      }
      if (!url) throw 'missing url for go()';

      compatMessagePost(id, msgContent);
      setTimeout(function() {
        processingGo = false;
  ...
  var STORAGE_MESSAGE_KEY = '__message__';
  ...
  function compatMessagePost(id, content) {
    ...
    msgList.push({from: 'fake', content: content});
    messages[id] = msgList;

    localStorage[STORAGE_MESSAGE_KEY] = JSON.stringify(msgList);
  }
